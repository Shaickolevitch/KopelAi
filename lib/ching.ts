...
export async function createCheckoutSession({
  ...
  successUrl: string;
  cancelUrl: string;
  // Optional Ching discount codes to apply (e.g. the student coupon). Ching
  // validates them and applies the discount to the price at checkout.
  couponCodes?: string[];
}) {
  ...
  if (couponCodes && couponCodes.length > 0) body.coupon_codes = couponCodes;
  return chingFetch('/checkout_sessions', body);
}
...
// The student rate is a Ching discount coupon applied to the normal plan at
// checkout (not a separate price) — people request the code from us directly.
