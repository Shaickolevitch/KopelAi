...
export default function PlanPage() {
  ...
  return (
    <div className="max-w-2xl mx-auto w-full px-4 sm:px-6 py-10">
      ...
      {/* Install KopelAi as an app (PWA) — platform-aware instructions, always here */}
      <div className="mt-6 rounded-xl border border-stone-200 dark:border-zinc-800 bg-white dark:bg-zinc-900 p-5">
        <div className="flex items-center gap-2 mb-2">
          <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="text-indigo-700 dark:text-indigo-300">
            <rect x="5" y="2" width="14" height="20" rx="2" /><path d="M12 18h.01" />
          </svg>
          <div className="font-semibold text-stone-900 dark:text-zinc-100">{isHebrew ? 'התקנת קופלAI כאפליקציה' : 'Install KopelAi as an app'}</div>
        </div>
        <p className="text-sm text-stone-500 dark:text-zinc-400 mb-3">
          {isHebrew ? 'גישה מהירה ממסך הבית או משולחן העבודה, כמו אפליקציה אמיתית — בלי להוריד כלום.' : 'Quick access from your home screen or desktop, like a real app — nothing to download.'}
        </p>
        <InstallInstructions />
      </div>
