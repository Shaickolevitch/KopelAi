...
// Sub-nav for the שיחה area: live conversation vs. searchable history.
export default function ChatTabs({ active }: { active: 'chat' | 'history' }) {
  const { language } = useLang();
  const he = language === 'he';
  const item = (key: 'chat' | 'history', href: string, label: string) => (
    <Link
      href={href}
